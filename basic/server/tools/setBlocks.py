...
def validate_args(args):
    ...
    if args.blockType < 0 or args.blockType > 255:
        raise ValueError(
            f"blockTypeは0以上255以下の整数である必要があります。指定値: {args.blockType}"
        )
    ...
    for name, value in [("y0", args.y0), ("y1", args.y1)]:
        if value < 0 or value > 255:
            raise ValueError(
                f"{name}の値が範囲外です。0から255の間である必要があります。指定値: {value}"
            )
...
def main():
    ...
    # 座標の開始点を定義
    parser.add_argument(
        "--x0", type=int, required=True, help="X coordinate of the starting point"
    )
    parser.add_argument(
        "--y0", type=int, required=True, help="Y coordinate of the starting point"
    )
    parser.add_argument(
        "--z0", type=int, required=True, help="Z coordinate of the starting point"
    )

    # 座標の終了点を定義
    parser.add_argument(
        "--x1", type=int, required=True, help="X coordinate of the ending point"
    )
    parser.add_argument(
        "--y1", type=int, required=True, help="Y coordinate of the ending point"
    )
    parser.add_argument(
        "--z1", type=int, required=True, help="Z coordinate of the ending point"
    )

    # ブロックタイプを定義
    parser.add_argument(
        "--blockType", type=int, required=True, help="Type of block to set (0-255)"
    )
    ...
    try:
        ...
        # ブロックを設置
        mc.setBlocks(
            args.x0, args.y0, args.z0, args.x1, args.y1, args.z1, args.blockType
        )
