...
def parse_arguments():
    ...
    if len(sys.argv) == 1:
        parser.print_help()
        logger.warning(
            "コマンドライン引数が指定されていません。--mcp オプションが必要です。"
        )
        sys.exit(1)
...
def load_mcp_clients():
    ...
    if args.mcp is None:
        logger.error(
            "MCP設定ファイルが指定されていません。--mcp オプションを使用してください"
        )
        raise ValueError(
            "MCP設定ファイルが指定されていません。--mcp オプションを使用してください"
        )
    ...
    try:
        ...
        for key in mcp_settings.keys():
            ...
            try:
                ...
                logger.error(
                    f"MCPクライアント '{key}' の初期化中にエラーが発生しました: {e}"
                )
                raise RuntimeError(
                    f"MCPクライアント '{key}' の初期化に失敗しました: {e}"
                )
...
def strands_callback_handler(**kwargs):
    ...
    if "data" in kwargs:
        ...
        # ツール名が変わった場合のみ表示する
        if (
            not hasattr(strands_callback_handler, "last_tool")
            or strands_callback_handler.last_tool != tool_name
        ):
        ...
        with ExitStack() as stack:
            ...
            for i, client in enumerate(clients):
                try:
                    ...
                    logger.error(
                        f"クライアント {i+1}/{len(clients)} からツールの取得に失敗しました: {e}"
                    )

            # ツールが取得できなかった場合の警告
            if len(tools) <= 2:  # capture と image_reader だけの場合
                logger.warning(
                    "Minecraftツールが取得できませんでした。基本ツールのみで実行します。"
                )
